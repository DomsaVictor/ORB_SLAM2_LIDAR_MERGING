...
float KeyFrame::ComputeRealDepth(cv::Mat points_x, cv::Mat points_y)
{
    ...
    cv::Mat orb_points_mat_y;
    for(int i=0; i<N; i++)
    {
        if(vpMapPoints.at(i))
        {
            orb_points_mat_x.push_back((float)vpMapPoints.at(i)->GetWorldPos().at<float> (2)); // x
            orb_points_mat_y.push_back((float) -1 * vpMapPoints.at(i)->GetWorldPos().at<float> (0)); // y
            m++;
        }
    }
    cout << "M =    [" << m << "]\n";

    if (points_x.size().area() < orb_points_mat_x.size().area())
    {
        m = points_x.size().area();
    }
    else 
    {
        m = orb_points_mat_x.size().area();
    }
    
    cout << "M =    [" << m << "]\n";

    // cv::Mat lidar_points_mat_x(m, 1, CV_32F, distance);
    points_x = points_x(cv::Range(0, m-1), cv::Range::all());
    points_y = points_y(cv::Range(0, m-1), cv::Range::all());

    cout << points_x.size().area();

    {
        cv::Mat aux_prod_x_1 = points_x * orb_points_mat_x.t();
        float aux_prod_fx_1 = cv::sum(aux_prod_x_1)[0];
        lambda_x = (float) m * aux_prod_fx_1 - (cv::sum(points_x)[0] * cv::sum(orb_points_mat_x)[0]);
        cv::Mat aux_prod_x_2 = orb_points_mat_x * orb_points_mat_x.t();
        float aux_prod_fx_2 = sum(aux_prod_x_2)[0];
        lambda_x = lambda_x / ((float) m * aux_prod_fx_2 - (cv::sum(orb_points_mat_x)[0] * cv::sum(orb_points_mat_x)[0]));
    }

    {
        cv::Mat aux_prod_y_1 = points_y * orb_points_mat_y.t();
        float aux_prod_fy_1 = cv::sum(aux_prod_y_1)[0];
        lambda_y = (float) m * aux_prod_fy_1 - (cv::sum(points_y)[0] * cv::sum(orb_points_mat_y)[0]);
        cv::Mat aux_prod_y_2 = orb_points_mat_y * orb_points_mat_y.t();
        float aux_prod_fy_2 = sum(aux_prod_y_2)[0];
        lambda_y = lambda_y / ((float) m * aux_prod_fy_2 - (cv::sum(orb_points_mat_y)[0] * cv::sum(orb_points_mat_y)[0]));
    }

    cout << "IN COMPUTE DISTANCES LAMBDA X: [" << lambda_x << "]\n";
    cout << "IN COMPUTE DISTANCES LAMBDA Y: [" << lambda_y << "]\n";
    cout << "IN COMPUTE DISTANCES LAMBDA M: [" << (lambda_x + lambda_y) /2 << "]\n";
    
    return (lambda_x + lambda_y) /2;
}
